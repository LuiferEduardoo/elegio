...
2. **Transcribe** with the **OpenAI audio API** (`gpt-4o-transcribe`, verbose_json segments). Long audios are split with ffmpeg and their timestamps re-aligned to the original timeline. Known Whisper-style hallucinations (e.g. the "Subtítulos … Amara.org" loop on music) are filtered out by [`hallucinations.py`](app/domains/transcription/hallucinations.py) before alignment, regardless of the model used.
