"""CLI: embed document chunks and upsert them into the Qdrant ``document_chunks`` collection.

Run after ``scripts.chunk_documents``. Only chunks missing from Qdrant are
...
from app.domains.document_chunk.embedding import (
    embed_and_upsert,
    fetch_chunks,
    get_all_chunk_ids,
...
def main() -> None:
    ...
    ensure_collection(qdrant, DOCUMENT_COLLECTION_NAME)
    embedder = Embedder()

    with engine.connect() as conn:
        all_ids = get_all_chunk_ids(conn)
        print(f"Total document chunks in MySQL: {len(all_ids)}")
