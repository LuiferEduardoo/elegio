"""CLI: embed news chunks and upsert them into the Qdrant ``news_chunks`` collection.

Run after ``scripts.chunk_news``. Only chunks missing from Qdrant are embedded,
...
from app.domains.news_chunk.embedding import (
    embed_and_upsert,
    fetch_chunks,
    get_all_chunk_ids,
...
def main() -> None:
    ...
    ensure_collection(qdrant, NEWS_COLLECTION_NAME)
    embedder = Embedder()

    with engine.connect() as conn:
        all_ids = get_all_chunk_ids(conn)
        print(f"Total news chunks in MySQL: {len(all_ids)}")
